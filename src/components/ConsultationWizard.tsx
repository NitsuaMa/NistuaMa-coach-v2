...
import { calculateStartingWeight, Gender, AgeGroup, SkillLevel } from '../lib/consultation-utils';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Info, Play, FileText, ChevronRight, Stethoscope, CheckSquare, Dumbbell, User, X, Trash2, Plus } from 'lucide-react';
...
export function ConsultationWizard({ client, machines, authTrainer, trainers, onComplete, onCancel }: ConsultationWizardProps) {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'sessionNotes'), {
        ...
        note: `Demo Consultation. Age: ${ageGroup}, Skill: ${skillLevel}. Goals: ${goals}`,
        createdAt: serverTimestamp()
      });
  ...
  return (
    <div className="flex flex-col bg-[#0A2E46] min-h-screen text-white pb-32">
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-4 flex flex-col gap-4 mt-8">
            {routine.map((machineName, idx) => {
              const weight = calculateStartingWeight(machineName, gender, ageGroup, skillLevel);
                      ...
                      <div className="flex justify-between items-start mb-2 gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <select 
                            ...
                            onChange={(e) => {
                              const newRoutine = [...routine];
                              newRoutine[idx] = e.target.value;
                              setRoutine(newRoutine);
                            }}
                            className="bg-transparent text-3xl font-black uppercase italic tracking-tighter text-[#0F172A] leading-none mb-2 outline-none appearance-none cursor-pointer w-full"
                          >
                            {machines.map(m => (
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <Button 
                            ...
                            className="text-rose-400 hover:text-rose-600 hover:bg-rose-100/50 shrink-0 h-14 w-14 rounded-xl"
                          >
      ...
      {/* Bottom Fixed Action */}
      <div className="fixed bottom-0 left-0 right-0 p-8 pt-16 bg-gradient-to-t from-[#0A2E46] via-[#0A2E46] to-transparent pointer-events-none flex justify-end items-end z-40">
        <Button 
